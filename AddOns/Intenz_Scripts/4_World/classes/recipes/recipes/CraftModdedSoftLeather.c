class CraftModdedSoftLeather extends RecipeBase
{		
	override void Init()
	{
		m_Name = "Add liquid to dried leather";
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"DriedLeather");//you can insert multiple ingredients this way
		...
		m_IngredientAddQuantity[1] = -500;// 0 = do nothing
		m_IngredientDestroy[1] = false;// false = do nothing
		m_IngredientUseSoftSkills[1] = false;// set 'true' to allow modification of the values by softskills on this ingredient
		//----------------------------------------------------------------------------------------------------------------------
		
		//result1
		AddResult("SoftLeather");//add results here
	...
	override bool CanDo(ItemBase ingredients[], PlayerBase player)//final check for recipe's validity
	{
		...
		if ( container.GetLiquidType() & (LIQUID_WATER | LIQUID_RIVERWATER | LIQUID_BEER) )
		{
			return true;
		}
		return false;
